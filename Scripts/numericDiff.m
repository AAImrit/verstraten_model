function derivative = numericDiff (y_val, x_val)
    ...
    if (data_length < 3)
        derivative(2) = (y_val(2)-y(1))/(x_val(2) - x(1));
        return;
    end

    derivative(1) = (y_val(1) - y(2))/( x_val(2) - x(1) ); %forward difference
    derivative(data_length) = (y_val(data_length) - y(data_length-1)) / (x_val(data_length) - x(data_length-1)); %backward difference
    derivative(2:data_length-1) = (y_val(1:data_length-2) - y(3:data_length))./(x_val(1:data_length-2) - x(3:data_length)); %central difference
end